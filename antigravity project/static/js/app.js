document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateHistory() {
        ...
        fb.onSnapshot(q, (snapshot) => {
            ...
            historyDocs.sort((a, b) => {
                const dateA = a.timestamp ? (typeof a.timestamp.toDate === 'function' ? a.timestamp.toDate() : new Date(a.timestamp)) : 0;
                const dateB = b.timestamp ? (typeof b.timestamp.toDate === 'function' ? b.timestamp.toDate() : new Date(b.timestamp)) : 0;
                return dateB - dateA;
            });
            ...
            historyDocs.forEach(item => {
                ...
                if (item.timestamp) {
                    if (typeof item.timestamp.toDate === 'function') {
                        dateStr = item.timestamp.toDate().toLocaleString();
                    } else {
                        // Handle string format from migration
    ...
    chatForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                addChatMessage(data.error || "I'm having trouble connecting to my brain right now.", 'bot');
            }
        } catch (error) {
            console.error('Chat error:', error);
            removeTypingIndicator();
            addChatMessage("Server error. Please check your connection.", 'bot');
        }
    });
    ...
    function startReminderCheck() {
        ...
        notificationCheckInterval = setInterval(() => {
            ...
            console.log(`Checking reminders at ${currentTimeStr}. Active reminders:`, activeReminders.map(r => r.time));
            ...
            activeReminders.forEach(r => {
                if (r.time === currentTimeStr) {
                    ...
                    if ('speechSynthesis' in window) {
                        ...
                        msg.text = `Attention! It is time to take your medicine: ${r.medicine_name}. Please take it now.`;
    ...
    if (testAlarmBtn) {
        testAlarmBtn.addEventListener('click', () => {
            console.log("Testing alarm and speech...");
            const audio = new Audio('https://assets.mixkit.co/active_storage/sfx/2869/2869-preview.mp3');
            audio.play().catch(e => alert("Please interact with the page first!"));
            
            if ('speechSynthesis' in window) {
                const msg = new SpeechSynthesisUtterance("Test announcement: This is how your medicine reminder will sound.");
                window.speechSynthesis.speak(msg);
            }
            showToastNotification("Test Medicine", "Now");
        });
    }
